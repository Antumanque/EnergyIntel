#!/usr/bin/env python3
"""
Parser para Formularios SUCTD en formato XLSX/XLS.

Este parser extrae datos estructurados de los formularios SUCTD
...
- Versión template: 2504-FORM-SUCTD-V1
...
class SUCTDXLSXParser:
    ...
    def parse(self, xlsx_path: str) -> Dict[str, Any]:
        """
        Parsea un archivo XLSX de Formulario SUCTD.
        ...
        self.logger.info(f"📊 Parseando XLSX SUCTD: {xlsx_file.name}")

        try:
            # Abrir workbook
            wb = openpyxl.load_workbook(xlsx_path, data_only=True)

            # Buscar hoja "FORMULARIO SUCTD" (puede tener variaciones)
            sheet = None
            for sheet_name in wb.sheetnames:
                if 'FORMULARIO' in sheet_name.upper() and 'SUCTD' in sheet_name.upper():
                    sheet = wb[sheet_name]
            ...
            if sheet is None:
                ...
                self.logger.warning(f"⚠️  No se encontró hoja 'FORMULARIO SUCTD', usando primera hoja: {sheet.title}")
            ...
            self.logger.error(f"❌ Error parseando XLSX SUCTD: {e}", exc_info=True)
            raise
    ...
    def _parse_worksheet(self, sheet: Worksheet) -> Dict[str, Any]:
        ...
        Estructura del XLSX SUCTD (basada en docs reales, no en template):
        ...
        modo_operacion_inversores, ni algunos otros del schema.
        ...
        data["modo_operacion_inversores"] = None
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Uso: python -m src.parsers.xlsx_suctd <ruta_xlsx>")
        sys.exit(1)
    ...
    try:
        ...
        print("\n" + "=" * 70)
        print("📊 DATOS EXTRAÍDOS DEL FORMULARIO SUCTD (XLSX)")
        print("=" * 70)
