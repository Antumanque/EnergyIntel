#!/usr/bin/env python3
"""
Script de parseo masivo de formularios Fehaciente (PDF y XLSX).

Este script:
1. Obtiene lista de documentos Fehaciente descargados pero NO parseados
2. Parsea cada documento (PDF o XLSX)
3. Guarda resultados en base de datos
4. Genera estadísticas de éxito/error

Uso:
    python -m src.batch_parse_fehaciente [--limit N] [--dry-run]

Ejemplos:
    # Parsear primeros 10 documentos (prueba)
    python -m src.batch_parse_fehaciente --limit 10

    # Parsear todos los documentos
    python -m src.batch_parse_fehaciente

    # Ver qué se parsearía sin ejecutar
    python -m src.batch_parse_fehaciente --dry-run
...
class FehacienteBatchParser:
    """Parseador masivo de formularios Fehaciente."""
    ...
    def get_pending_documents(self, limit: int = None) -> List[Dict]:
        """
        Obtiene lista de documentos Fehaciente descargados pero NO parseados.
        ...
        logger.info(f"📋 Encontrados {len(docs)} documentos Fehaciente pendientes de parsear")
    ...
    def parse_document(self, doc: Dict) -> Tuple[bool, str]:
        ...
        try:
            ...
            success = self.db.parse_and_store_fehaciente_document(
    ...
    def run_batch_parsing(self, limit: int = None, dry_run: bool = False) -> Dict:
        ...
        """
        logger.info("=" * 70)
        logger.info("🚀 PARSEO MASIVO DE FORMULARIOS Fehaciente")
        logger.info("=" * 70)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parseo masivo de formularios Fehaciente",
...
Ejemplos:
  ...
  # Ver qué se parsearía sin ejecutar
  python -m src.batch_parse_fehaciente --dry-run --limit 50
        """
    )
    ...
    batch_parser = FehacienteBatchParser()
